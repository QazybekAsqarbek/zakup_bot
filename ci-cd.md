...
Руководство по настройке автоматического развертывания zakup_bot на сервер Yandex Cloud при каждом push в ветку `main`.
...
yc container registry create --name zakup-bot-registry
...
# Получите IP адрес
yc compute instance get zakup-bot-server --format json | jq -r .network_interfaces[0].primary_v4_address.one_to_one_nat.address
```
...
| `TELEGRAM_TOKEN` | Токен Telegram бота | @BotFather |
| `ANTHROPIC_API_KEY` | Токен Anthropic API | https://console.anthropic.com/ |
| `DEEPSEEK_API_KEY` | Токен DeepSeek API | https://platform.deepseek.com/ |
| `ADMIN_CHAT` | ID чата для уведомлений о деплое | @userinfobot |
| `MONGO_URL` | URL подключения к MongoDB | По умолчанию: `mongodb://mongo:27017` (для docker-compose) |
...
sudo mkdir -p ~/zakup-bot/data ~/zakup-bot/logs
sudo chmod -R 755 ~/zakup-bot
```
...
docker ps | grep smart_procure_bot

# Посмотрите логи
docker logs smart_procure_bot
...
```bash
# Последние 50 строк
docker logs --tail 50 smart_procure_bot

# В реальном времени
docker logs -f smart_procure_bot

# Логи из файла
ssh ubuntu@<your_server_ip> 'cat ~/zakup-bot/logs/debug.log | tail -50'
```
...
```bash
# Статус контейнера
docker ps -a | grep smart_procure_bot

# Использование ресурсов
docker stats smart_procure_bot

# Информация о контейнере
docker inspect smart_procure_bot
```
...
**Решение:**
1. Посмотрите логи: `docker logs smart_procure_bot`
2. Проверьте все Secrets — особенно `TELEGRAM_TOKEN`, `ANTHROPIC_API_KEY` и `DEEPSEEK_API_KEY`
3. Проверьте, что директории созданы: `ls -la ~/zakup-bot/`
...
**Решение:**
1. Проверьте логи бота: `docker logs smart_procure_bot`
2. Проверьте `TELEGRAM_TOKEN` — правильный ли токен
3. Проверьте `ANTHROPIC_API_KEY` и `DEEPSEEK_API_KEY`
4. Проверьте переменные окружения в контейнере:
   ```bash
   docker exec smart_procure_bot env | grep -E 'TELEGRAM_TOKEN|ANTHROPIC_API_KEY|DEEPSEEK_API_KEY'
   ```
...
**Решение:**
- База сохраняется в volume `~/zakup-bot/data`
- Убедитесь, что в docker-compose.prod.yml правильно смонтирован volume:
  ```yaml
  volumes:
    - ./data:/app/data
  ```
...
docker stats smart_procure_bot --no-stream
```
...
docker stop smart_procure_bot
docker rm smart_procure_bot

# Посмотрите доступные образы
docker images | grep zakup-bot

# Запустите предыдущую версию через docker-compose
cd ~/zakup-bot
# Отредактируйте docker-compose.prod.yml, указав нужный тег образа
